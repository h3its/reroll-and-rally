<script>
  ...
  [p.title, p.excerpt ?? '', (p.tags ?? []).join(' '), p.author ?? '']
    .join(' ')
    .toLowerCase()
  ...
  {#if filtered.length}
    <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
      {#each filtered as post}
        <a href={`/wiki/${post.slug}`} class="card bg-neutral shadow-md hover:shadow-lg transition">
          {#if post.cover}
            <figure>
          ...
          <div class="card-body">
            ...
            <h2 class="card-title mt-1">{post.title}</h2>
